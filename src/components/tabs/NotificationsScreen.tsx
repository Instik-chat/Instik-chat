...
export const NotificationsScreen: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto pb-24 text-white p-2">
      <h2 className="text-xl font-black text-emerald-400 mb-4 px-2">Notifications</h2>

      <div className="flex flex-col gap-2">
        {notifications.length === 0 ? (
          <div className="text-slate-500 text-xs text-center py-12">No notifications yet.</div>
        ) : (
          notifications.map((n) => (
            <div key={n.id} className="p-3.5 bg-slate-900 border border-slate-800 rounded-2xl flex items-center gap-3">
              <div className="p-2 rounded-full bg-slate-950 text-emerald-400">
                {n.type === 'like' && <Heart className="w-4 h-4 fill-rose-500 text-rose-500" />}
                {n.type === 'comment' && <MessageCircle className="w-4 h-4 text-emerald-400" />}
