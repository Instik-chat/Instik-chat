...
export const SearchDiscovery: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto pb-24 text-white p-2">
      <div className="flex flex-col gap-3 sticky top-14 z-20 bg-[#050505]/95 p-2 rounded-2xl backdrop-blur-md border border-[#1A1A1A] shadow-xl">
        <form onSubmit={handleSearch} className="relative">
          <Search className="absolute left-3.5 top-3.5 w-4 h-4 text-[#34D399]" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search INSTIK CHAT or Google..."
            className="w-full pl-10 pr-10 py-3 bg-[#121212] border border-[#1F1F1F] focus:border-[#34D399] rounded-2xl text-xs text-white placeholder-gray-500 outline-none"
          />
          <button
            type="submit"
            className="absolute right-2 top-2 px-3 py-1 bg-[#34D399] text-black font-bold rounded-xl text-xs hover:bg-[#2EB886] transition-all"
          >
            Search
          </button>
        </form>
        ...
        <div className="flex gap-1.5 overflow-x-auto scrollbar-none text-xs font-semibold">
          <button
            onClick={() => setMode('all')}
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-colors ${mode === 'all' ? 'bg-[#34D399] text-black font-bold' : 'bg-[#121212] text-gray-400 hover:text-white'}`}
          >
            All
          </button>
          <button
            onClick={() => setMode('google')}
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap flex items-center gap-1 transition-colors ${mode === 'google' ? 'bg-[#34D399] text-black font-bold' : 'bg-[#121212] text-sky-400'}`}
          >
            <Globe className="w-3 h-3" /> Google Search
          </button>
          <button
            onClick={() => setMode('users')}
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-colors ${mode === 'users' ? 'bg-[#34D399] text-black font-bold' : 'bg-[#121212] text-gray-400 hover:text-white'}`}
          >
            People
          </button>
          <button
            onClick={() => setMode('reels')}
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-colors ${mode === 'reels' ? 'bg-[#34D399] text-black font-bold' : 'bg-[#121212] text-gray-400 hover:text-white'}`}
          >
            Reels
          </button>
        </div>
      ...
      {loading && (
        <div className="text-center py-12 text-[#34D399] text-xs font-semibold">
            ...
            <div className="flex flex-col gap-3">
              ...
              {results.google.map((g, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-3.5 bg-[#0F0F0F] border border-[#1A1A1A] hover:border-sky-500/50 rounded-2xl flex flex-col gap-1 transition-all"
                >
                  <div className="flex items-center justify-between text-xs font-bold text-sky-300">
                    <span className="truncate">{g.title}</span>
                    <ArrowUpRight className="w-3.5 h-3.5 shrink-0" />
                  </div>
                  <p className="text-[11px] text-gray-400 line-clamp-2">{g.snippet}</p>
                </a>
              ))}
            </div>
          )}
            ...
            <div className="flex flex-col gap-2">
              ...
              {results.users.map((u) => (
                <div
                  ...
                  className="p-3 bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl flex items-center gap-3 cursor-pointer hover:bg-[#121212] transition-colors"
                >
                  <img src={u.avatar} alt="" className="w-10 h-10 rounded-full object-cover border border-[#1F1F1F]" />
                  <div>
